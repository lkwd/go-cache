...
func TestCacheTimes(t *testing.T) {
	...
	tc := New(Expiration(50*time.Millisecond), CleanupInterval(1*time.Millisecond))
	tc.Set("a", 1, DefaultExpiration)
...
func TestNewFrom(t *testing.T) {
	...
	tc := New(Expiration(DefaultExpiration), InitialItems(m))
	a, found := tc.Get("a")
	if !found {
		t.Fatal("Did not find a")
	}
...
func TestOnEvicted(t *testing.T) {
	...
	tc := New(Expiration(DefaultExpiration), EvictionCallback(func(k string, v interface{}) {
		if k == "foo" && v.(int) == 3 {
			works = true
		}
	}))
...
func testFillAndSerialize(t *testing.T, tc *Cache) {
	...
	oc := New(Expiration(DefaultExpiration))
	err = oc.Load(fp)
	if err != nil {
...
func TestGetWithExpiration(t *testing.T) {
	...
	if item, ok := tc.getItem("e"); ok && item.Expiration != expiration.UnixNano() {
