...
func TestShardedCache(t *testing.T) {
	tc := unexportedNewSharded(Expiration(DefaultExpiration), Shards(13))
...
func benchmarkShardedCacheGet(b *testing.B, exp time.Duration) {
	b.StopTimer()
	tc := unexportedNewSharded(Expiration(exp), Shards(10))
...
func benchmarkShardedCacheGetManyConcurrent(b *testing.B, exp time.Duration) {
	b.StopTimer()
	n := 10000
	tsc := unexportedNewSharded(Expiration(exp), Shards(20))
	keys := make([]string, n)
	for i := 0; i < n; i++ {
